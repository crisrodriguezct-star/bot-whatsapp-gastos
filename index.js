    ...
    if (body.object && body.entry?.[0]?.changes?.[0]?.value?.messages?.[0]) {
      ...
      if (msg.type === 'text') {
        ...
        if (sesionActual && sesionActual.esperandoBusquedaPrecio) {
          ...
          const resultados = await buscarHistoricoPrecios(textBody);
        ...
        if (respuestaId?.startsWith('OBRA_')) {
          ...
          sesionActual.obra = obraMap[respuestaId] || 'Suc. Otro';

          await enviarBotones(from, `🏗️ *Obra:* ${sesionActual.obra}\n\n📌 *Selecciona la Categoría Principal:*`, [
          ...
          sesionActual.categoria = '31) HONORARIOS';
          sesionActual.concepto = `${sesionActual.concepto} (Honorarios a ${beneficiarioMap[respuestaId]})`;

          await desplegarFormasPago(from);

        } else if (respuestaId?.startsWith('PAY_')) {
          if (respuestaId === 'PAY_Efectivo') {
            sesionActual.metodo = 'Efectivo';
            await pedirFactura(from);
          } else if (respuestaId === 'PAY_Transf') {
            sesionActual.metodo = 'Transferencia';
          ...
          sesionActual.subMetodo = subMap[respuestaId] || '';
          await pedirFactura(from);

        } else if (respuestaId?.startsWith('FAC_')) {
          if (respuestaId === 'FAC_Si') {
            sesionActual.estatusFactura = 'Facturado 🟢';
          } else if (respuestaId === 'FAC_Pendiente') {
            sesionActual.estatusFactura = 'Pendiente 🟡';
          } else {
            sesionActual.estatusFactura = 'No Requiere 🔴';
          }

          const alerta = await verificarSobregiroContratista(sesionActual.obra, sesionActual.categoria, sesionActual.concepto, sesionActual.monto);

          await guardarEnSheets(sesionActual);

          const metodoTexto = sesionActual.subMetodo ? `${sesionActual.metodo} (${sesionActual.subMetodo})` : sesionActual.metodo;
          let resumen = `✅ *Gasto Registrado con Éxito*\n\n` +
            `🆔 *ID:* ${sesionActual.idMovimiento}\n` +
            `👤 *Registró:* ${sesionActual.usuario}\n` +
            `📌 *Categoría:* ${sesionActual.categoria}\n` +
            `💵 *Monto:* ${formatoMoneda(sesionActual.monto)}\n` +
            `📝 *Concepto:* ${sesionActual.concepto}\n` +
            `🏗️ *Obra:* ${sesionActual.obra}\n` +
            `💳 *Pago:* ${metodoTexto}\n` +
            `📄 *Factura:* ${sesionActual.estatusFactura}`;
